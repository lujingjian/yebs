...
public class WebSockeConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        ...
        //配置代理域 可以配置多个 前缀为 /queue 可以在配置域上向客户端推送消息
        ...
        registration.interceptors(new ChannelInterceptor() {
            ...
            public Message<?> preSend(Message<?> message, MessageChannel channel) {
                ...
                if (StompCommand.CONNECT.equals(accessor.getCommand())) {
                    // 令牌
                    String token = accessor.getFirstNativeHeader("Auth-Token");//Auth-Token是前端定义的参数

                    if (!StringUtils.isEmpty(token)) {
                        String authToken = token.substring(tokenHead.length());//拿到完整令牌
                        // 从 token 中获取用户名
                        ...
                        if (!StringUtils.isEmpty(username)) {//判断用户名是否为空
                            ...
                            //验证token是否有效 有效重新设置用户对象
                            if (jwtUtil.dataToken(authToken,userDetails)) {
